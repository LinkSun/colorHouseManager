    ...
    export default {
        ...
        methods: { 
            getList() {
                ...
                httpClient.get('admin/building/house/detail/' + this.id).then(response => {
                    ...
                    this.makeFormData()
                    this.form.images = this.dealFormPhoto(this.house.images != '' ? this.house.images.split(',') : [])
                    this.fileList = this.showPhoto(this.house.images != '' ? this.house.images.split(',') : [])
                    this.listLoading = false
                })
            },
            makeFormData() {
                for (const key in this.form) {
                    this.form[key] = this.house[key]
                }
            },
            dealFormPhoto(arr) {
                if (arr.length == 0) {
            ...
            onSubmit() {
                ...
                httpClient.post('admin/building/house/update/' + this.id, this.form).then(response => {
                    this.$message({
                        type: 'success',
                        message: '操作成功'
                    })
